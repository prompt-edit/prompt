...
from model_utils.functions import predict_next_word,pytorch_cos_sim
    ...
    def __init__(self, opt,editor):
        ...
        if self.opt.style_mode=='plm':
            if 'gpt-j-hf' in self.opt.class_name:
                self.plm=GPTNeoForCausalLM.from_pretrained(self.opt.class_name,revision="float16")
                self.plm.half()
            else:
                self.plm =AutoModelForCausalLM.from_pretrained(self.opt.class_name,revision="float16")
            self.plm.eval()
            self.plm.to(device)
