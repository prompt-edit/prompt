...
def main():
    ...
    with open('../data/{}/train.0'.format(opt.dataset),'r') as f:
        for line in f.readlines():
            line=line.lower()
            train_src.append(tokenizer.encode(line.lower().strip())[:opt.max_len])
    with open('../data/{}/train.1'.format(opt.dataset),'r') as f:
        for line in f.readlines():
            train_tgt.append(tokenizer.encode(line.lower().strip())[:opt.max_len])
    with open('../data/{}/valid.0'.format(opt.dataset),'r') as f:
        for line in f.readlines():
            valid_src.append(tokenizer.encode(line.lower().strip())[:opt.max_len])
    with open('../data/{}/valid.1'.format(opt.dataset),'r') as f:
        for line in f.readlines():
            valid_tgt.append(tokenizer.encode(line.lower().strip())[:opt.max_len])
